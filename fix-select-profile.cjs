...
export default function SelectProfile() {
  ...
  async function handleLink() {
    ...
    try {
      ...
      // Re-fetch the member directly and update auth context via query invalidation
      qc.invalidateQueries({ queryKey: ["members"] });
      // Force a fresh session check which will re-fetch the member
      const { data: { session } } = await supabase.auth.getSession();
      if (session) {
        // Trigger a page reload after a short delay to let Supabase session persist
...
console.log('SelectProfile.tsx updated');