...
import { LayoutDashboard, Trophy, Activity, User, ShieldAlert, LogOut } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "linear-gradient(rgba(10,10,10,0.82) 0%, rgba(10,10,10,0.88) 100%), url('/gym-bg.png') center/cover no-repeat fixed" }}>
      <header className="sticky top-0 z-50 bg-black/80 backdrop-blur border-b border-white/10">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <a href="/" className="flex items-center gap-3">
            ...
            <span className="font-display text-white text-2xl tracking-wider uppercase hidden sm:block">Ironworks Fitness</span>
          </a>
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 ml-2 pl-2 border-l border-white/10">
              {member && <span className="text-xs text-white/40 hidden md:block">{member.name}</span>}
              <button onClick={signOut} title="Sign out" className="p-2 rounded-lg text-white/40 hover:text-white hover:bg-white/5 transition-colors">
      ...
      <footer className="border-t border-white/5 py-4 text-center text-xs text-white/20">Ironworks Fitness · Forge Challenge · 12-Week Program</footer>
    </div>
  );
}
