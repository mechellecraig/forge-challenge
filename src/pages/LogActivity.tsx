...
export default function LogActivity() {
  ...
  const { currentWeek, loading: weekLoading } = useCurrentWeek();
  const [week, setWeek] = useState<number>(1);

  // Seed the week selector with the current program week when it loads
  useEffect(() => {
    if (!weekLoading && currentWeek) {
      setWeek(currentWeek);
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!member) return;
    if (hasDuplicate && !confirmOverwrite) {
  ...
  const hrTarget = member
    ? Math.round((220 - member.age) * scoring.hr_threshold)
    : 0;
      ...
      <div className="max-w-2xl mx-auto pb-12">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
          ...
          <p className="text-white/50 text-lg">
            Account not linked to a member yet.
          </p>
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12 space-y-6">
      {toast && (
        <div
          className={
            "fixed top-4 right-4 z-50 px-5 py-3 rounded-xl font-bold text-sm shadow-lg " +
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#111] border border-white/20 rounded-2xl p-6 max-w-sm w-full">
            ...
            <p className="text-white/50 text-sm mb-6">
              A log for{" "}
              <span className="text-white font-semibold">{DAYS[dayIndex]}</span>{" "}
              already exists this week. This will replace it.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmOverwrite(false)}
                className="flex-1 py-2.5 rounded-lg border border-white/20 text-white/60 text-sm font-bold"
              >
                Cancel
              </button>
              <button
                onClick={doSubmit}
                disabled={saving}
                className="flex-1 py-2.5 rounded-lg bg-primary text-white text-sm font-bold"
              >
      ...
      <div>
        ...
        <p className="text-white/50 mt-2">
          Submit your daily grind to add points to your team's score.
        </p>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl px-5 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm">
            {selectedTeam?.name} · Age {member.age} · HR zone target: {hrTarget}
            + bpm
          </p>
        </div>
          ...
          {(
            [
              [
                Footprints,
                "Walk / Run",
                `${scoring.walk} pts per mile`,
                "text-blue-400",
              ],
              [
                Bike,
                "Bike",
                `${scoring.bike} pt${scoring.bike !== 1 ? "s" : ""} per mile`,
                "text-yellow-400",
              ],
              [
                UtensilsCrossed,
                "Meal Plan (Mon–Fri)",
                `${scoring.meal_weekday} pts per day`,
                "text-orange-400",
              ],
              [
                UtensilsCrossed,
                "Meal Plan (Sat–Sun)",
                `${scoring.meal_weekend} pts per day`,
                "text-orange-400",
              ],
              [
                HeartPulse,
                "HR Zone Session",
                `+${scoring.hr_zone} pts (≥${Math.round(scoring.hr_threshold * 100)}% max HR, >30 min)`,
                "text-red-400",
              ],
            ] as [any, string, string, string][]
          ).map(([Icon, label, hint, color]) => (
            <div
              key={label}
      ...
      >
        ...
        <div>
          <h3 className="text-sm font-bold uppercase tracking-wider text-white/40 border-b border-white/10 pb-2 mb-4">
            Activity Miles
          </h3>
          <div className="grid grid-cols-2 gap-4">
            {(
              [
                ...
                [
                  "Bike",
                  `${scoring.bike} pt${scoring.bike !== 1 ? "s" : ""}/mi`,
                  bike,
                  setBike,
                ],
              ] as [string, string, string, (v: string) => void][]
            ).map(([name, hint, val, setter]) => (
              <div key={name}>
                <label className={lbl}>
                  {name}{" "}
                ...
                <input
                  ...
                  className={inp + " font-mono text-lg text-center"}
                  placeholder="0"
                />
              </div>
            ))}
            ...
            {parseFloat(avgHr) > 0 && (
              <p
                className={
                  ...
                  (hrHit ? "text-green-400" : "text-orange-400")
                }
              >
                {hrHit
                  ? "HR zone hit — +5 pts"
                  : hrTarget - parseFloat(avgHr) + " bpm below target"}
              </p>
            )}
          </div>
          <div>
            ...
            <label className="flex items-start gap-3 p-4 rounded-xl border border-white/10 bg-white/[0.02] cursor-pointer">
              ...
              <div>
                <span className="text-white text-sm font-semibold block">
                  Followed meal plan today
                </span>
                <span className="text-white/40 text-xs">
                  {dayIndex >= 5 ? "+5 pts (weekend)" : "+3 pts (weekday)"}
                </span>
              </div>
            </label>
          </div>
