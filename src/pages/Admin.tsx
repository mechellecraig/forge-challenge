    ...
    return (
      <div className="max-w-md mx-auto mt-20">
        <div className="bg-white/5 border border-primary/30 rounded-2xl p-8 shadow-[0_0_30px_rgba(255,85,0,0.1)]">
          ...
          <h1 className="text-2xl font-display font-bold uppercase tracking-wider text-white text-center mb-1">
            Admin Access
          </h1>
          <p className="text-white/40 text-sm text-center mb-6">
            Restricted area. Enter PIN to continue.
          </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/50 mt-1">
            Manage teams, members, bonuses and settings.
          </p>
        </div>
        <button
...
function MemberActivity() {
  ...
  const getTeamName = (tid: string) =>
    teams?.find((t) => t.id === tid)?.name || "Unknown";
  ...
  const memberTotalPoints = (memberId: string) => {
    ...
    if (!member) return 0;
    return memberLogs(memberId).reduce(
      (sum, log) =>
        sum +
        calcDayPoints(
          {
            ...
            >
              <div className="flex items-center gap-4 text-left">
                ...
                <div>
                  ...
                  <div className="text-xs text-primary font-bold uppercase tracking-wider mt-0.5">
              ...
              <div className="flex items-center gap-6">
                <div className="text-right hidden sm:block">
                  ...
                  <div className="text-xs text-white/30">
                    {mLogs.length} log{mLogs.length !== 1 ? "s" : ""}
                  </div>
                </div>
                {isOpen ? (
                    ...
                    <table className="w-full text-sm min-w-[550px]">
                      <thead>
                        <tr className="bg-black/30 text-xs uppercase tracking-wider text-white/30">
                          ...
                          <th className="px-4 py-2 text-right font-bold text-white/60">
                            Pts
                          </th>
                        </tr>
                      </thead>
                        ...
                        {mLogs.map((log, i) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 py-2.5 text-white/60">
                                {DAYS[log.day_index] ?? log.day_index}
                              </td>
                              <td className="px-4 py-2.5 text-right text-white/50">
                                {log.walk > 0 ? `${log.walk}mi` : "—"}
                              </td>
                              <td className="px-4 py-2.5 text-right text-white/50">
                                {log.run > 0 ? `${log.run}mi` : "—"}
                              </td>
                              <td className="px-4 py-2.5 text-right text-white/50">
                                {log.bike > 0 ? `${log.bike}mi` : "—"}
                              </td>
              ...
              {editingId === t.id ? (
                ...
                <div className="flex items-center justify-between p-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => startEdit(t)}
                      className="text-white/30 hover:text-white p-1.5 rounded hover:bg-white/10 transition-colors"
                    >
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => handleDelete(t.id)}
                      className="text-red-400/60 hover:text-red-400 p-1.5 rounded hover:bg-red-400/10 transition-colors"
                    >
...
function ManageBonuses() {
  ...
  const inp =
    "w-full bg-black/40 border border-white/10 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-primary";
  const inpSm =
    "bg-black/40 border border-white/10 text-white rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:border-primary";
  const lbl =
    "block text-xs uppercase tracking-wider text-white/40 font-bold mb-1.5";
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8">
      ...
      <div className="bg-white/5 border border-primary/20 rounded-xl p-6">
        <h2 className="font-display font-bold uppercase tracking-wider text-primary mb-1">
          Award Bonus
        </h2>
        <p className="text-white/30 text-xs mb-4">
          Grant extra points for challenges, spirit, or attendance.
        </p>
        <form onSubmit={handleAdd} className="space-y-4">
          <div>
            <label className={lbl}>Team</label>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Week 4 tournament winner"
              className={inp}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className={`${inp} text-primary font-mono`}
              />
            </div>
          </div>
          <button
              ...
              {editingId === b.id ? (
                ...
                <div className="p-3 space-y-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`${inpSm} w-full text-primary font-mono font-bold`}
                    />
                  </div>
                ...
                <div className="flex items-center justify-between px-3 py-3 gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-white/30 uppercase tracking-wider mt-0.5">
                      Week {b.week}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded hover:bg-red-400/10 text-red-400/50 hover:text-red-400 transition-colors"
                      title="Delete"
                    >
...
function ScoringSettings() {
  const qc = useQueryClient();
  const { data: current = DEFAULT_SCORING } = useQuery({
    queryKey: ["scoring"],
    queryFn: getScoringConfig,
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const hrPct = parseFloat(
      val("hr_threshold_pct", current.hr_threshold * 100),
    );

    if (
      [walkRun, bike, mealWd, mealWe, hrZone].some((v) => isNaN(v) || v < 0)
    ) {
      setMsg({ text: "All point values must be 0 or greater.", ok: false });
      return;
    }
    if (isNaN(hrPct) || hrPct < 50 || hrPct > 100) {
      setMsg({ text: "HR threshold must be between 50 and 100.", ok: false });
      return;
    }
    setSaving(true);
    try {
      await setScoringConfig({
        walk_run: walkRun,
        bike,
        meal_weekday: mealWd,
        meal_weekend: mealWe,
        hr_zone: hrZone,
        hr_threshold: hrPct / 100,
      });
  ...
  const inp =
    "w-full bg-black/40 border border-white/10 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-primary font-mono text-center";
  const lbl =
    "block text-xs uppercase tracking-wider text-white/40 font-bold mb-1.5";
    ...
    <div className="max-w-lg">
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        ...
        <p className="text-white/30 text-xs mb-6">
          Adjust point values for all activity types. Changes apply to all
          future calculations and leaderboard totals.
        </p>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-white/50 mb-3 border-b border-white/10 pb-2">
              Activity Miles
            </p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="0.5"
                  value={val("walk_run", current.walk)}
                  onChange={(e) => set("walk_run", e.target.value)}
                  className={inp}
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="1"
                  value={val("meal_weekday", current.meal_weekday)}
                  onChange={(e) => set("meal_weekday", e.target.value)}
                  className={inp}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="1"
                  value={val("meal_weekend", current.meal_weekend)}
                  onChange={(e) => set("meal_weekend", e.target.value)}
                  className={inp}
                />
              </div>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  max="100"
                  step="1"
                  value={val("hr_threshold_pct", current.hr_threshold * 100)}
                  onChange={(e) => set("hr_threshold_pct", e.target.value)}
                  className={inp}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min="0"
                  step="1"
                  value={val("hr_zone", current.hr_zone)}
            ...
            <p className="text-white/30 text-xs mt-2">
              Members earn the bonus points when their logged avg HR is at or
              above this % of their max HR (220 − age).
            </p>
          </div>
          {msg && (
            <p
...
function ChangePin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (next.length < 4) {
      setMsg({ text: "PIN must be at least 4 characters.", ok: false });
      return;
    }
    if (next !== confirm) {
    ...
    try {
      ...
      setMsg({ text: err.message || "Failed to update PIN.", ok: false });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="max-w-sm">
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        ...
        <p className="text-white/30 text-xs mb-5">
          Update the PIN used to access admin settings.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className={lbl}>New PIN</label>
            <input
              type="password"
              value={next}
