    ...
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-5">
            ...
            {(["password", "magic"] as Mode[]).map(m => (
              <button key={m} onClick={() => { setMode(m); setMsg(null); }}
                className={`flex-1 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-colors ${
                  mode === m ? "bg-primary text-white" : "text-white/40 hover:text-white"
                }`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full h-12 rounded-xl bg-primary text-white font-display font-bold text-base uppercase tracking-wider disabled:opacity-40 hover:bg-primary/90 transition-colors shadow-[0_0_20px_rgba(255,85,0,0.2)]">
        ...
        <p className="text-center text-xs text-white/20">
          Access is by invitation only. Contact your team admin if you need access.
        </p>
      </div>
    </div>
  );
